...
export default function SignUp(){
    ...
    const handleSubmit = e => {
        e.preventDefault()
        axios.post("http://localhost:3000/signup",{name,email,password}).then(result =>{console.log(result) 
        navigate('/login')}).catch(e=>console.log(e))
    }
    return <div className="main h-screen w-screen flex items-center content-end justify-center">
        <div className="shadow-xl h-[600px] w-[900px] rounded-3xl bg-white flex items-end justify-center">
           ...
           <h1 className="text-xl font-bold text-gray-600">Book Harbor</h1>
            ...
            <p className="mt-1 text-white text-sm mb-6">If You Are A New User Please Fill Your Details</p>
            <form action="POST" className="flex flex-col mt-2 gap-4 text-white">
                <input className="h-10 p-2 rounded tracking-wider bg-glass-dark text-white" type="text" name="name" value={name} placeholder="Name" onChange={e => setName(e.target.value)} />
                ...
                <button className="h-10 p-2 rounded tracking-wider bg-gray-600 text-white" onClick={handleSubmit}>Submit</button>
                
                <div className="flex gap-3 text-white text-sm "><p>Already A User?</p>
